...
const IcuReservation = () => {
  const [icureservation, setICURes] = useState()
  ...
  const fetchAdmit = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8080/room/inpatient/today",
        {
          headers: headers,
        }
      );
      setICURes(response.data);
    } catch (error) {
